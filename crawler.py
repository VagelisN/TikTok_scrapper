...
def getVideoIdsBasedOnKeyword(name, offset):
    ids = []
    headers = {
        "accept": "*/*",
...
def getTopVideosForHashtag(hashtagName):
    ...
    allVideos = []
    for x in getVideoIdsBasedOnKeyword(hashtagName, 0):
        time.sleep(0.2)
        video = getVideoFromId(x)
        videoDict = video.as_dict
        views = videoDict['stats']['playCount']
        likes = videoDict['stats']['diggCount']
        shares = videoDict['stats']['shareCount']
        video_score = views + 1.6 * likes + 2.2 * shares
        allVideos.append({"id" : video.id, "score" : video_score, "likes": likes, "views": views, "shares": shares})
...
with TikTokApi(logging_level=logging.DEBUG, custom_verify_fp= customVerify, generate_static_device_id=True) as api:
    for x in getVideoIdsBasedOnKeyword('challenge', 0): 
        '''
        We need to get each specific hashtag that contains 
        the word 'challenge', without being an exact match
        '''

        temp = getChallengeHashtagsFromVideo(getVideoFromId(x))
